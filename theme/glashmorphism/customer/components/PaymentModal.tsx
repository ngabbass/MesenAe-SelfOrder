...
export default function PaymentModal({
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Container Dialog */}
      <div className="glass-panel-heavy w-full max-w-md rounded-[1.5rem] shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-300 flex flex-col max-h-[85vh] overflow-hidden border border-white/20 dark:border-white/10">
        
        {/* Header Modal */}
        <div className="bg-transparent px-6 py-5 border-b border-white/20 dark:border-white/10 flex items-center justify-between sticky top-0 z-20 shrink-0">
          ...
          <button 
            onClick={onClose}
            disabled={processing}
            className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 dark:bg-slate-800/40 backdrop-blur-sm flex items-center justify-center text-slate-500 transition-colors shrink-0 disabled:opacity-50"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-transparent">
          ...
          {/* Callout Total Harga */}
          <div className="bg-blue-50/50 dark:bg-blue-950/20 p-5 rounded-2xl border border-blue-100 dark:border-blue-900/40 text-center">
          ...
          {selectedMethod.category === 'qris' && selectedMethod.qrisString && (() => {
            ...
            return (
              <div className="flex flex-col items-center justify-center space-y-5">
                <div className="bg-slate-50 dark:bg-white p-1 rounded-3xl shadow-sm border border-slate-200 w-full max-w-[280px]">
                  <QrisCard 
                ...
                <div className="text-center space-y-3">
                  ...
                  <button 
                    ...
                    className="mx-auto px-5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-2 active:scale-95 transition-all shadow-sm"
                  >
                    <QrCode size={16} />
              ...
              <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-4">
                ...
                {selectedMethod.accountNumber && (
                  <div>
                    ...
                    <div className="flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3.5 py-2.5 rounded-xl">
                      ...
                      <button 
                        ...
                        className={`p-2 rounded-lg active:scale-95 transition-all border shrink-0 ${
                          copied 
                            ? 'text-emerald-600 bg-emerald-50 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-transparent' 
                            : 'text-blue-600 bg-blue-50 border-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:border-transparent'
                        }`}
                        title="Salin Nomor"
                      >
        ...
        <div className="p-5 border-t border-white/20 dark:border-white/10 bg-transparent shrink-0">
          <button 
            onClick={onConfirm}
            disabled={processing}
            className="w-full bg-[#25D366] hover:bg-[#20bd5a] disabled:bg-slate-300 dark:disabled:bg-slate-800 text-white rounded-xl py-3.5 font-bold text-sm flex justify-center items-center gap-2 active:scale-[0.98] transition-all shadow-md shadow-[#25D366]/20"
          >
            {processing ? (
