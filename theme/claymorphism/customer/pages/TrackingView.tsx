...
export default function TrackingView({ 
  ...
  if (!liveTx) {
    return (
      <div className={isEmbedded ? "flex-1 flex flex-col items-center justify-center p-6 bg-transparent py-16 animate-in fade-in duration-300" : "flex-1 flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-slate-950 min-h-screen animate-in fade-in duration-300"}>
        <div className="w-24 h-24 bg-blue-500/10 dark:bg-slate-900/50 rounded-full flex items-center justify-center mb-6 shadow-inner border border-blue-500/20">
          <PackageOpen size={40} strokeWidth={1.5} className="text-blue-500 dark:text-blue-400" />
        </div>
        ...
        <button 
          ...
          className="mt-8 clay-btn-primary px-8 py-3.5 text-sm font-bold uppercase tracking-wider"
        >
          Lihat Katalog Menu
        </button>
      </div>
  ...
  return (
    <div className={isEmbedded ? "flex-1 flex flex-col bg-transparent animate-in fade-in duration-300" : "flex-1 flex flex-col bg-slate-50 dark:bg-slate-950 min-h-screen animate-in fade-in duration-300"}>
              ...
              {activeTxs.map((tx, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`snap-center shrink-0 px-5 py-2.5 rounded-full font-bold text-sm transition-all border-none ${
                      isSelected 
                        ? 'clay-btn-primary' 
                        : 'clay-card py-2 px-5 text-slate-600 dark:text-slate-300 hover:scale-[1.02]'
                    }`}
                  >
            ...
            {activeTxs.map((tx, idx) => {
              ...
              return (
                <button
                  key={tx.id}
                  onClick={() => handleTabClick(tx)}
                  className={`snap-center shrink-0 px-5 py-2.5 rounded-full font-black text-xs sm:text-sm uppercase tracking-wider transition-all border-none ${
                    isSelected 
                      ? 'clay-btn-primary' 
                      : 'clay-card py-2 px-5 text-slate-600 dark:text-slate-300 hover:scale-[1.02]'
                  }`}
      ...
      <div className={isEmbedded ? "p-4 pb-4" : "flex-1 p-5 overflow-y-auto pb-24 custom-scrollbar-hide"}>
        ...
        <div className="clay-card p-6 mb-6 flex justify-between items-center relative overflow-hidden border-none text-slate-800 dark:text-white">
          ...
          {/* Decorative blur */}
          <div className="absolute -right-4 -bottom-4 w-32 h-32 bg-blue-500/5 rounded-full blur-2xl pointer-events-none"></div>
          ...
          <div className="relative z-10">
            ...
            <p className="font-black text-2xl tracking-tight text-slate-800 dark:text-white">
              {liveTx?.receipt_number || liveTx?.receiptNumber || 'TX-???'}
            </p>
          </div>
          
          <div className="text-right relative z-10 border-l border-slate-200 dark:border-slate-800 pl-5 flex flex-col items-end">
            {(() => {
              const tbl = liveTx?.table_number || liveTx?.tableNumber || '';
              const parsedTable = parseTableNumber(tbl);
              return (
                <>
                  <div className="flex items-center justify-end gap-1.5 text-blue-600 dark:text-blue-400 mb-1.5">
                    <MapPin size={14} />
                  ...
                  <div className="flex flex-col items-end gap-1">
                    {parsedTable.isTakeAway ? (
                      <div className="bg-blue-500/10 p-2 rounded-xl backdrop-blur-sm shadow-inner"><TakeawayIcon className="w-8 h-8 text-blue-500 dark:text-blue-400 drop-shadow-sm" /></div>
                    ) : (
                      <div className="flex flex-col items-end gap-1.5">
                        <span className="inline-flex items-center gap-1 bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 text-[10px] font-extrabold px-3 py-1.5 rounded-xl border border-blue-100 dark:border-blue-900 tracking-wide uppercase leading-none shadow-sm">
        ...
        {!isPaid ? (
          ...
          <div className="clay-card p-6 mb-6 border-none text-center space-y-6">
            <div className="w-16 h-16 bg-blue-50 dark:bg-blue-500/10 rounded-full flex items-center justify-center mx-auto text-blue-500 animate-pulse shadow-inner">
              <RpIcon size={32} />
            </div>
            ...
            <div className="pt-2 text-xs font-bold text-blue-600 dark:text-blue-400 flex items-center justify-center gap-1.5 animate-pulse">
          ...
          <div className="clay-card p-6 mb-6 border-none relative">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full clay-btn-primary py-4 text-sm mb-1"
          >
          ...
          <button
            ...
            className="w-full clay-btn-success py-4 text-sm font-bold flex items-center justify-center"
          >
          ...
          <button 
            onClick={() => setView('menu')} 
            className="w-full clay-btn-secondary py-4 text-sm"
          >
