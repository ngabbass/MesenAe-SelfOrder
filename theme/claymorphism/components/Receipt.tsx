...
export default function Receipt({ open, onClose, transaction, items, storeSettings, paymentMethodName }: ReceiptProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-md md:max-w-xl w-[95vw] max-h-[90vh] overflow-y-auto p-6 flex flex-col clay-card">
        ...
        <div className="grid grid-cols-2 gap-3 mt-2">
          <button 
            className="flex flex-col items-center justify-center gap-2 h-20 clay-btn-secondary text-slate-700 dark:text-slate-200" 
            onClick={handleDownload} 
            disabled={downloading || sharing}
          >
            {downloading ? <Loader2 className="w-6 h-6 animate-spin text-blue-500" /> : <Download className="w-6 h-6 text-slate-500 hover:text-blue-500" />}
          ...
          <button 
            ...
            disabled={downloading || sharing}
          >
            {sharing ? <Loader2 className="w-6 h-6 animate-spin text-blue-500" /> : <Share2 className="w-6 h-6 text-slate-500 hover:text-blue-500" />}
            <span className="text-[10px] font-bold uppercase tracking-wider">Bagikan</span>
          </button>
        </div>

        <button 
          className="w-full mt-4 clay-btn-primary py-3.5 text-base font-bold uppercase" 
          onClick={onClose}
        >
          Tutup
        </button>
