...
export default function MenuDetailSheet({ item, hideImage = false, onClose, onAdd, onDirectBuy }: MenuDetailSheetProps) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-end sm:items-center justify-center transition-all duration-300 ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      ...
      {/* Sheet Content */}
      <div 
        className={`relative w-full max-w-md glass-panel-heavy sm:rounded-[2rem] rounded-t-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transform transition-transform duration-300 ease-out ${
      ...
      >
        ...
        <div className="absolute top-0 left-0 right-0 pt-3 pb-2 flex justify-center z-20 pointer-events-none sm:hidden">
          <div className={`w-12 h-1.5 rounded-full ${hideImage ? 'bg-slate-300 dark:bg-slate-700' : 'bg-white/40 backdrop-blur-md'}`} />
        </div>

        {/* Close Button overlay */}
        <button 
          onClick={handleClose} 
          className={`absolute top-4 right-4 z-20 p-2.5 rounded-full transition-all ${
            hideImage 
              ? 'bg-white/20 hover:bg-white/30 dark:bg-slate-800/40 backdrop-blur-sm text-slate-500 shadow-sm' 
        ...
        <div className="flex-1 overflow-y-auto pb-[140px] custom-scrollbar">
          ...
          <div className={`p-6 space-y-6 ${hideImage ? 'pt-12' : ''}`}>
            ...
            <div className={hideImage ? 'pr-10' : ''}>
              ...
              {item.sku && (
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400 bg-white/10 dark:bg-slate-800/40 border border-white/20 dark:border-white/10 inline-block px-2.5 py-1 rounded-md">
                  SKU: {item.sku}
                </p>
              )}
            </div>
            ...
            {item.variants?.length > 0 && (
              <div className="space-y-5">
                {item.variants.map((group: ProductVariantGroup, gIdx: number) => (
                  <div key={gIdx} className="glass-card border border-white/20 dark:border-white/10 rounded-[1.5rem] p-5">
                          ...
                          <button 
                            ...
                            className={`w-full flex items-center justify-between p-3.5 rounded-xl border-2 transition-all ${
                              isSelected 
                                ...
                                : 'glass-card border-white/20 dark:border-white/10 hover:border-white/30'
                            }`}
                          >
            ...
            <div>
              ...
              <textarea 
                ...
                className="w-full glass-card border border-white/20 dark:border-white/10 rounded-2xl p-4 text-sm focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none transition-all dark:text-white placeholder-slate-400"
                rows={3}
              ></textarea>
            </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 glass-panel border-t border-white/20 dark:border-white/10 px-6 py-4 z-30 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 16px)' }}>
