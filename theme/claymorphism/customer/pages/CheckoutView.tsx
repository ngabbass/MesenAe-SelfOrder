...
const OrderInfoBlock = React.memo(({ customerName, tableNumber }: { customerName?: string | null, tableNumber?: string | number }) => {
  ...
  return (
    <div className="clay-card border-none mx-4 p-5 mt-4">
...
const CartItemListBlock = React.memo(({ cart, updateCartQty, onAddMore }: { cart: CartItem[], updateCartQty?: (id: string | number, delta: number) => void, onAddMore: () => void }) => {
  return (
    <div className="clay-card border-none mx-4 p-5">
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 flex flex-col justify-center min-w-0 py-0.5">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-1">
                    ...
                    {updateCartQty && (
                      <div className="flex items-center bg-slate-100/50 dark:bg-slate-800/50 p-1 rounded-full border border-slate-200/40 dark:border-slate-700/40 backdrop-blur-sm shadow-inner">
                        <button 
                          onClick={() => updateCartQty(item.cartId || item.id, -1)} 
                          className="w-6 h-6 rounded-full clay-btn-secondary flex items-center justify-center text-slate-600 dark:text-slate-300 active:scale-95 transition-all border-none"
                        >
                        ...
                        <button 
                          onClick={() => updateCartQty(item.cartId || item.id, 1)} 
                          className="w-6 h-6 rounded-full clay-btn-primary flex items-center justify-center text-white active:scale-95 transition-all border-none"
                        >
    ...
    <div className="flex-1 flex flex-col bg-slate-50 dark:bg-slate-950 min-h-screen font-sans animate-in fade-in duration-300">
      ...
      <div className="flex-1 flex flex-col pb-6">
        ...
        <div className="clay-card border-none mx-4 p-5">
          ...
          <div className="flex gap-2.5">
            <input
              ...
              className="flex-1 clay-input px-4 py-3 border-none text-sm font-semibold text-slate-900 dark:text-white placeholder-slate-400"
            />
            <button
              onClick={handleApplyPromo}
              className="clay-btn-primary px-5 py-3 border-none text-sm font-bold shadow-sm"
            >
              Terapkan
            </button>
          </div>
          ...
          {appliedVoucher && (
            <div className="mt-4 p-3.5 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800/50 flex items-center justify-between">
              ...
              <button 
                ...
                className="clay-btn-secondary px-3 py-1.5 border-none text-xs font-bold text-slate-500 hover:text-red-500 transition-colors"
              >
                Hapus
              </button>
            </div>
        ...
        {storeSettings?.enableSplitBill !== false && (
          <>
            <div className="clay-card border-none mx-4 p-5 flex items-center justify-between">
              <div className="flex items-center gap-3.5">
                ...
                {splitConfig?.active ? (
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="clay-btn-secondary px-3 py-2 border-none text-xs font-bold text-slate-700 dark:text-slate-300 hover:scale-[1.02] transition-all"
                    >
                      Ubah
                    </button>
                    <button 
                      onClick={() => { if(setSplitConfig) setSplitConfig(null); toast.info('Dimatikan'); }} 
                      className="clay-btn-secondary px-3 py-2 border-none text-red-600 text-xs font-bold hover:bg-red-500/10 hover:scale-[1.02] transition-all"
                    >
                      Batal
                    </button>
                  </div>
                  ...
                  <button 
                    onClick={() => setSplitModalOpen(true)} 
                    className="clay-btn-secondary px-5 py-2 border-none min-w-[80px] text-blue-600 dark:text-blue-400 font-bold text-xs hover:scale-[1.02] transition-all"
                  >
                    Atur
                  </button>
                )}
        ...
        {!splitConfig?.active && (
          <>
            <div 
              className="clay-card border-none mx-4 p-5 cursor-pointer hover:scale-[1.01] transition-transform flex items-center justify-between" 
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {selectedMethod ? (
                  <button className="clay-btn-secondary flex items-center gap-2 border-none px-3 py-1.5 hover:scale-[1.02] transition-all">
                    <PaymentLogoBlock 
                  ...
                  <button className="clay-btn-secondary px-5 py-2 border-none min-w-[80px] font-bold text-blue-600 dark:text-blue-400 text-xs hover:scale-[1.02] transition-all">
                    Pilih
                  </button>
                )}
              </div>
        ...
        <div className="mt-auto px-5 pt-8 pb-4">
          <button 
            ...
            className="w-full h-[52px] clay-btn-primary border-none text-white rounded-2xl font-bold text-sm flex justify-center items-center gap-2.5 active:scale-[0.98]"
          >
            {processing ? (
              <>
      ...
      <AlertDialog open={clearConfirmOpen} onOpenChange={setClearConfirmOpen}>
        <AlertDialogContent className="max-w-[340px] w-[90vw] p-6 clay-card">
          <AlertDialogHeader>
          ...
          <AlertDialogFooter className="mt-6 flex flex-row gap-3">
            <AlertDialogCancel className="flex-1 mt-0 h-12 font-bold clay-btn-secondary flex items-center justify-center">
              Tutup
            </AlertDialogCancel>
            <AlertDialogAction 
              onClick={() => { setCart([]); setClearConfirmOpen(false); setView('menu'); }} 
              className="flex-1 h-12 font-bold clay-btn-primary bg-red-500 hover:bg-red-600 text-white shadow-none border-none flex items-center justify-center"
            >
              Hapus
            </AlertDialogAction>
          </AlertDialogFooter>
