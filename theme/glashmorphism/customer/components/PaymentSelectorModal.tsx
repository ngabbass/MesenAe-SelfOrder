  ...
  return createPortal(
    <div className={`fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div 
        ...
        onClick={onClose} 
      />
      <div className={`glass-panel-heavy w-full max-w-md rounded-[1.5rem] shadow-2xl relative z-10 flex flex-col max-h-[85vh] overflow-hidden border border-white/20 dark:border-white/10 transition-all duration-300 transform ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
              ...
              <div 
                ...
                className={`flex items-center justify-between p-4 rounded-2xl border-2 cursor-pointer transition-all active:scale-[0.98] ${
                  isSelected 
                    ...
                    : 'border-transparent glass-card hover:border-white/35 dark:hover:border-white/20 shadow-sm'
                }`}
              >
