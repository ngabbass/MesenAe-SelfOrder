...
Berikut saya lampirkan bukti pembayarannya. Mohon segera dikonfirmasi. Terima kasih!`
  ...
  return (
    <div className={isEmbedded ? "flex-1 flex flex-col bg-transparent animate-in fade-in duration-300" : "flex-1 flex flex-col bg-slate-50 dark:bg-slate-950 h-screen animate-in fade-in duration-300"}>
          ...
          <div className="space-y-3 sm:space-y-4">
            {history.slice(0, visibleCount).map((tx: Transaction) => {
              ...
              return (
                <div 
                  key={tx.id} 
                  className="group clay-card !rounded-2xl p-4 sm:p-5 relative overflow-hidden active:scale-[0.98] cursor-pointer transition-all"
                  onClick={async () => {
            ...
            {history.length > visibleCount && (
              <button 
                ...
                className="w-full clay-btn-secondary !rounded-2xl py-3.5"
              >
                Lihat Selanjutnya
              </button>
            )}
