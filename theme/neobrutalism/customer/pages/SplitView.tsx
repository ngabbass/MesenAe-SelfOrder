...
const SectionSpacer = React.memo(() => (
  <div className="h-2.5 w-full bg-[#ffc700] border-y-2 border-black" />
));
    ...
    <div className="flex-1 flex flex-col bg-transparent min-h-screen font-sans animate-in fade-in duration-300">
      ...
      <div className="sticky top-0 z-30 bg-white dark:bg-slate-900 border-b-3 border-black dark:border-slate-700 px-4 py-3.5 flex items-center justify-between">
        <button 
          ...
          className="w-11 h-11 flex items-center justify-center bg-[#ffc700] hover:bg-[#ffe066] text-black border-2 border-black dark:border-slate-700 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_#374151] active:translate-x-[0.5px] active:translate-y-[0.5px] transition-all shrink-0 disabled:opacity-50"
          aria-label="Kembali"
        >
          <ChevronLeft size={20} strokeWidth={3} />
        </button>
        <h1 className="font-black text-sm uppercase tracking-wider text-black dark:text-white tracking-tight absolute left-1/2 -translate-x-1/2">
          Bayar Split Bill
        </h1>
      ...
      <div className="flex-1 flex flex-col pb-[120px]">
        ...
        {/* Blok Rangkuman */}
        <div className="bg-white dark:bg-slate-900 px-5 pt-8 pb-6 flex flex-col items-center justify-center border-b-2 border-black dark:border-slate-700">
          <div className="w-14 h-14 bg-[#ff90e8] text-black border-2 border-black dark:border-slate-700 flex items-center justify-center mb-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_#374151]">
            <SplitSquareHorizontal size={26} strokeWidth={2.5} />
          </div>
          <p className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">
            Total Tagihan Keseluruhan
          </p>
          <p className="text-[32px] font-black text-black dark:text-white leading-none tracking-wider uppercase">
            {FORMAT_IDR(actualTotal)}
          </p>
          <p className="text-[10px] font-bold text-slate-600 dark:text-slate-400 mt-4 text-center max-w-[280px] uppercase leading-relaxed">
        ...
        <div className="bg-white dark:bg-slate-900 px-5 py-6">
          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-5 flex items-center gap-2">
            <RpIcon size={16} strokeWidth={2.5} /> Rincian Bagian
          </h3>
          ...
          <div className="space-y-4">
            {itemsList.map((portion: any, idx: number) => {
              ...
              return (
                <div 
                  key={portion.id} 
                  className={`relative overflow-hidden border-2 border-black dark:border-slate-700 transition-all duration-300 ${
                    isPaid 
                      ? 'bg-slate-100 dark:bg-slate-800 opacity-85 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_#374151]' 
                      : 'bg-white dark:bg-slate-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_#374151] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] dark:hover:shadow-[6px_6px_0px_0px_#374151]'
                  }`}
                >
                  <div className="p-5 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2.5 mb-1.5">
                        ...
                        <span className="px-2 py-0.5 border border-black text-[9px] font-black uppercase bg-[#ff90e8] text-black flex items-center gap-1.5">
                          {portion.paymentMethod === 'qris' ? (
                            <><QrCode size={11} strokeWidth={2.5} /> QRIS</>
                          ) : (
                            <><RpIcon size={11} strokeWidth={2.5} /> Tunai</>
                          )}
                        </span>
                      </div>
                      <p className={`text-lg font-black tracking-wider uppercase ${isPaid ? 'text-slate-400 dark:text-slate-500 line-through' : 'text-black dark:text-white'}`}>
                    ...
                    <div>
                      {isPaid ? (
                        <div className="flex items-center gap-1.5 text-black bg-[#a3e635] border border-black px-3 py-1.5 font-black text-[10px] uppercase">
                          <CheckCircle2 size={16} strokeWidth={2.5} /> Lunas
                        </div>
                      ) : (
                        <button
                          onClick={() => handlePayClick(idx)}
                          disabled={isProcessing || !txId}
                          className="flex items-center gap-2 bg-[#ffc700] hover:bg-[#ffe066] disabled:bg-slate-200 disabled:text-slate-400 border-2 border-black dark:border-slate-700 text-black px-5 py-2.5 font-black text-xs uppercase tracking-wider shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:shadow-[3px_3px_0px_0px_#374151] active:translate-x-[0.5px] active:translate-y-[0.5px] active:shadow-none transition-all"
                        >
                          {!txId ? (
          ...
          {paidSplits.every(Boolean) && (
            <div className="mt-8 animate-in fade-in zoom-in slide-in-from-bottom-4 duration-500">
              <div className="w-full bg-[#a3e635] text-black p-5 border-2 border-black dark:border-slate-700 font-black text-center text-xs uppercase tracking-wide shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_#374151] flex flex-col items-center justify-center gap-2">
                <div className="w-10 h-10 border-2 border-black dark:border-slate-700 bg-white text-black flex items-center justify-center mb-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_#374151]">
                  <Check strokeWidth={4} size={22} />
                </div>
                Semua Tagihan Dilunasi!
                <span className="text-[9px] font-bold opacity-90 uppercase">Memproses pesanan Anda...</span>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Centered Modal Instruksi Bayar Tunai */}
      {cashInstructionOpen && createPortal(
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm border-3 border-black dark:border-slate-700 text-black dark:text-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_#374151] relative z-10 animate-in fade-in zoom-in-95 duration-300 flex flex-col overflow-hidden">
            
            {/* Header Dialog */}
            <div className="bg-[#ffc700] px-6 py-5 border-b-2 border-black flex items-center justify-between shrink-0">
              <div>
                <h3 className="font-black text-sm uppercase tracking-wide text-black">Bayar Tunai</h3>
                <p className="text-[9px] font-black uppercase text-black/80 mt-0.5">Selesaikan di meja kasir</p>
              </div>
              <button 
                onClick={() => setCashInstructionOpen(false)} 
                className="w-8 h-8 border-2 border-black dark:border-slate-700 bg-white flex items-center justify-center text-black shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] dark:shadow-[1px_1px_0px_0px_#374151] hover:bg-[#fffdf0] shrink-0"
              >
            ...
            <div className="p-6 bg-white dark:bg-slate-900 space-y-4">
              <div className="w-14 h-14 bg-[#ff90e8] border-2 border-black dark:border-slate-700 flex items-center justify-center mx-auto mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_#374151]">
                <RpIcon className="w-7 h-7 text-black" strokeWidth={2.5} />
              </div>
              
              <p className="text-center text-[10px] font-bold uppercase text-slate-600 dark:text-slate-300 px-2 leading-relaxed">
                Tunjukkan Nomor Referensi ini ke kasir untuk proses pembayaran bagian Anda:
              </p>
              
              {/* Kartu Informasi Nominal */}
              <div className="bg-slate-100 dark:bg-slate-800 p-4.5 border-2 border-black dark:border-slate-700 space-y-2 relative overflow-hidden shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:shadow-[3px_3px_0px_0px_#374151]">
                <div className="absolute top-0 left-0 w-2 h-full bg-[#ffc700] border-r-2 border-black" />
                <p className="text-[9px] uppercase font-black text-slate-400 tracking-wider">Nomor Referensi</p>
                <p className="text-lg font-black text-black dark:text-white tracking-wider">{receiptNumber}</p>
                <div className="h-[1px] w-full border-t-2 border-dashed border-black dark:border-slate-700 my-2" />
                <p className="text-xs font-black uppercase text-slate-600 dark:text-slate-300">
                  Total Bayar: <span className="text-black dark:text-white font-black text-sm ml-1">{FORMAT_IDR(cashInstructionAmount)}</span>
                </p>
              </div>
              
              <p className="text-[9px] text-center italic text-slate-400 font-bold uppercase leading-relaxed pt-1">
                Tampilan web Anda akan otomatis diperbarui begitu kasir mengonfirmasi pembayaran uang tunai ini.
              </p>
            </div>
            
            {/* Footer Aksi Dialog */}
            <div className="p-5 bg-slate-100 dark:bg-slate-950 border-t-2 border-black shrink-0">
              <button 
                onClick={() => setCashInstructionOpen(false)} 
                className="w-full h-[48px] bg-[#ffc700] hover:bg-[#ffe066] text-black border-2 border-black dark:border-slate-700 font-black text-xs uppercase tracking-wider shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:shadow-[3px_3px_0px_0px_#374151] transition-all active:translate-x-[0.5px] active:translate-y-[0.5px] active:shadow-none"
              >
                Saya Mengerti
              </button>
            </div>
