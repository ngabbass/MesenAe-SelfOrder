...
export function QrisPaymentModal({
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-[92vw] sm:max-w-[400px] rounded-2xl p-0 overflow-hidden border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-2xl [&>button]:text-white [&>button]:hover:text-white/80 [&>button]:top-3 [&>button]:right-3 [&>button]:z-[110]">
        {/* ── Header Compact ── */}
            ...
            <div className="flex flex-col items-center py-2 gap-3 w-full">
              {isManual && qrisData ? (
                <div className="flex flex-col items-center gap-3 w-full animate-in fade-in zoom-in duration-300">
                  ...
                  <Button className="w-full font-bold h-11 rounded-xl bg-primary hover:bg-primary/90 text-white" onClick={() => onSuccessRef.current()}>
