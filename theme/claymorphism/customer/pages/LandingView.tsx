  ...
  return (
    <div className={`flex-1 overflow-y-auto ${isEmbedded ? 'pb-4' : 'pb-20'} bg-transparent font-sans animate-in fade-in duration-500 relative`}>
      ...
      <div className="px-5 pt-4 pb-14 relative z-10">
        <div className="flex justify-between items-center mb-0">
          ...
          <button 
            ...
            className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-900 flex items-center justify-center transition-all shrink-0 shadow-sm"
            aria-label="Toggle Dark Mode"
          >
            {isDarkMode ? <Sun size={20} className="text-blue-500" strokeWidth={2.5} /> : <Moon size={20} className="text-slate-700" strokeWidth={2.5} />}
          </button>
        </div>
      </div>

      {/* Floating Card: Posisi/Meja (Premium Clay) */}
      <div className="px-5 -mt-10 relative z-20 mb-6">
        <div className="clay-card p-4 flex items-center justify-between cursor-pointer group hover:border-blue-500/20 hover:scale-[1.01] active:scale-[0.99] transition-all" onClick={() => setView('others')}>
          <div className="flex items-center gap-3">
            <div className="w-[46px] h-[46px] rounded-xl bg-gradient-to-br from-blue-600 to-blue-500 flex items-center justify-center text-white shadow-sm shrink-0 group-hover:scale-105 transition-transform duration-300 clay-btn-primary p-0">
          ...
          <div className="w-9 h-9 rounded-full bg-blue-50 dark:bg-blue-950 flex items-center justify-center text-blue-500 group-hover:bg-blue-500 group-hover:text-white shrink-0 transition-colors shadow-sm">
      ...
      <div className="px-5 space-y-6">
                  ...
                  >
                    {displayOffers.map((promo, index) => (
                      <div key={`${promo.id || index}`} className="snap-center shrink-0 w-full px-4 md:px-6">
                        <PromoBanner
                          banner={promo}
                          className="w-full aspect-[21/9] md:aspect-[32/9] rounded-[2rem] shadow-xl shadow-blue-500/10 border border-white/20 dark:border-white/5"
                  ...
                  <div className="flex justify-center gap-1.5 mt-4">
                    {displayOffers.map((_, i) => (
                      <button
                        ...
                        className={`h-1.5 rounded-full transition-all duration-500 ${i === activeDotIndex ? 'w-6 bg-blue-500 shadow-md shadow-blue-500/30' : 'w-1.5 bg-white/40 dark:bg-slate-700 backdrop-blur-md'}`}
        ...
        <div>
          ...
          <div className="flex space-x-3.5 overflow-x-auto pb-3 custom-scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {categories.map((cat) => (
              ...
              >
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl clay-card flex items-center justify-center text-slate-700 dark:text-slate-300 transition-all duration-300 group-hover:bg-blue-500/10 group-hover:text-blue-600 dark:group-hover:text-blue-400 group-active:scale-90 shadow-sm border-none">
                  {cat.icon || <Package size={20} strokeWidth={2} />}
                </div>
                <span className="text-[9px] md:text-[11px] font-bold text-slate-600 dark:text-slate-400 text-center w-full truncate px-1 transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-400">
                  {cat.name}
                </span>
              </button>
            ))}
            ...
            <button 
              ...
              className="flex flex-col items-center space-y-1.5 min-w-[64px] group"
            >
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-white/10 dark:bg-black/20 backdrop-blur-md border border-slate-200 dark:border-slate-800 border-dashed flex items-center justify-center text-slate-500 transition-all duration-300 group-hover:border-blue-500/50 group-active:scale-90">
        ...
        {/* Populer / Rekomendasi (Premium Clay) */}
        <div>
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-black text-[19px] tracking-tight flex items-center text-slate-900 dark:text-white drop-shadow-sm">
              <Flame size={22} strokeWidth={2.5} className="text-blue-500 mr-2 drop-shadow-sm animate-pulse" /> 
              Produk Favorit
            </h3>
            <button onClick={() => setView('menu')} className="text-[13px] font-bold text-blue-600 hover:text-blue-700 dark:text-blue-400">
              Lihat Semua
            </button>
          </div>
            ...
            {sortedProducts.slice(0, 10).map((item) => {
              ...
              return (
                <div 
                  ...
                  className={`clay-card p-3 transition-all duration-300 shrink-0 snap-start w-[140px] sm:w-[160px] md:w-[180px] lg:w-[200px] group border-none ${
                    isOutOfStock 
                      ? 'opacity-60 grayscale cursor-not-allowed shadow-none' 
                      : 'hover:shadow-lg hover:-translate-y-1 cursor-pointer hover:border-blue-500/20'
                  }`}
                >
                  <div className="h-28 rounded-xl bg-slate-100 dark:bg-slate-800 mb-2.5 overflow-hidden relative flex items-center justify-center shadow-inner">
                    {item.photo ? (
                  ...
                  <div className="px-0.5 relative z-10">
                    ...
                    <div className="flex items-center justify-between gap-1 mt-2">
                      <p className="text-blue-600 dark:text-blue-400 font-black text-[13px]">
                      ...
                      {!isOutOfStock && (
                        <button 
                          ...
                          className="w-7 h-7 rounded-full flex items-center justify-center active:scale-95 transition-transform clay-btn-primary p-0 border-none"
                        >
