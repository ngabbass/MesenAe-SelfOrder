...
const ProductCard = React.memo(({ item, onSelect, onAddToCart, cartItemData, updateCartQty }: ProductCardProps) => {
  ...
  return (
    <div
      ...
      className={`clay-card p-0 overflow-hidden flex flex-col group transition-all duration-300 relative border-none ${
        isOutOfStock
          ? 'opacity-60 grayscale cursor-not-allowed'
          : 'cursor-pointer hover:-translate-y-1 hover:shadow-lg hover:border-blue-500/20'
      }`}
    >
      {/* Container Gambar */}
      ...
      <div className="flex-1 flex flex-col justify-between p-3 relative z-10 bg-white/40 dark:bg-slate-900/30 backdrop-blur-sm">
        <div className="mb-2">
        ...
        <div className="flex justify-between items-end mt-auto gap-2">
          <p className="font-black text-blue-600 dark:text-blue-400 text-[13px] tracking-tight">
            ...
            qty > 0 && !(item.variants && item.variants.length > 0) ? (
              <div className="flex items-center gap-1 flex-shrink-0 bg-slate-50 dark:bg-slate-800 rounded-full p-0.5 border border-slate-200 dark:border-slate-700 shadow-sm">
                <button
                  ...
                  className="w-6 h-6 rounded-full flex items-center justify-center bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 active:scale-90 shadow-sm transition-all border border-slate-100 dark:border-slate-600"
                ...
                <button
                  ...
                  className="w-6 h-6 rounded-full flex items-center justify-center clay-btn-primary active:scale-90 p-0 border-none shadow-sm transition-all"
              ...
              <button
                ...
                className="w-7 h-7 rounded-full flex items-center justify-center transition-all flex-shrink-0 bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 hover:bg-blue-500 hover:text-white active:scale-95 shadow-sm"
                aria-label={`Tambah ${item.name}`}
              >
...
export default function MenuView({
  ...
  return (
    <div className="flex-1 flex flex-col relative z-0">
      ...
      <div className={isEmbedded ? "pt-2 pb-3 px-4 relative z-10" : "sticky top-0 z-30 pt-4 pb-3 px-4 backdrop-blur-2xl bg-white/40 dark:bg-black/40 border-b border-slate-100 dark:border-slate-800/30 shadow-sm transition-all"}>
        {/* Soft Decorative Gradient behind header */}
        {!isEmbedded && <div className="absolute inset-0 bg-gradient-to-b from-blue-500/5 to-transparent -z-10 pointer-events-none" />}
        ...
        <div className="flex items-center gap-3 mb-4 relative z-10">
          {!isEmbedded && (
            <button
              ...
              className="w-11 h-11 flex items-center justify-center bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full shrink-0 text-slate-800 dark:text-white shadow-sm transition-all"
          ...
          <div className="flex-1 relative group">
            <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-500 dark:text-slate-400 group-focus-within:text-blue-500 transition-colors" size={18} strokeWidth={2.5} />
            <input
              type="text"
              placeholder="Cari menu favorit..."
              value={searchQuery}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setSearchQuery(e.target.value)}
              className="w-full clay-input pl-11 pr-4 py-3 rounded-2xl text-[15px] focus:outline-none transition-all placeholder-slate-500 dark:placeholder-slate-400 font-medium text-slate-900 dark:text-white border-none"
            />
            {searchQuery && (
              <button 
        ...
        <div className="flex gap-2.5 overflow-x-auto pb-1 custom-scrollbar-hide relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          <button
            ...
            className={`px-4 py-2 md:px-5 md:py-2.5 rounded-xl md:rounded-[1.2rem] text-[12px] md:text-[13px] font-bold whitespace-nowrap transition-all flex-shrink-0 active:scale-95 border-none ${
              selectedCategory === 'all'
                ? 'clay-btn-primary scale-[1.02]'
                : 'clay-card py-2 px-4 text-slate-700 dark:text-slate-200 hover:scale-[1.02]'
            }`}
          >
            Semua Menu
          </button>
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 md:px-5 md:py-2.5 rounded-xl md:rounded-[1.2rem] text-[12px] md:text-[13px] font-bold whitespace-nowrap transition-all flex items-center gap-1.5 md:gap-2 flex-shrink-0 active:scale-95 border-none ${
